...
class SecondViewController: UIViewController, CLLocationManagerDelegate {
    ...
    var lastInstruction2 = "hoi"
    
    let names2 = [ //text shown on screen when beacon is closest
    ...
    func locationManager(manager: CLLocationManager, didRangeBeacons beacons: [CLBeacon], inRegion region: CLBeaconRegion) {
        ...
        if (knownBeacons.count > 0){
            ...
            if((self.images2[closestBeacon.minor.integerValue]) != nil){
                let locationImage = UIImage(named: self.images2[closestBeacon.minor.integerValue]!)
                if(locationImage != nil){
                    self.naarStationImageView.image = locationImage
                }
            }
            
            if(self.names2[closestBeacon.minor.integerValue] != nil){
                self.naarStationTextView.text = self.names2[closestBeacon.minor.integerValue]
                if(self.names2[closestBeacon.minor.integerValue] != self.lastInstruction2){
                    UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, self.naarStationTextView.text)
                    self.lastInstruction2 = self.naarStationTextView.text
                }
            }
        }
    }
